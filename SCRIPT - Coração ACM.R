...
dados_cor <- dados_cor %>% 
  mutate(Categ_BC_Desc = case_when(BC_Descanço <= quantile(BC_Descanço, 0.25, na.rm = T) ~ "BC_descanço_baixo",
                                   BC_Descanço > quantile(BC_Descanço, 0.25, na.rm = T) & BC_Descanço <= quantile(BC_Descanço, 0.75, na.rm = T) ~ "BC_descanço_médio",
                                   BC_Descanço > quantile(BC_Descanço, 0.75, na.rm = T) ~ "BC_descanço_alto"))
...
dados_cor <- dados_cor %>% 
  select(-Idade, -BC_Descanço, -Colesterol, -BC_Max)
...
sjt.xtab(var.row = dados_cor$Doença_Card,
         var.col = dados_cor$ECG_Descanço,
         show.exp = TRUE,
         show.row.prc = TRUE,
         show.col.prc = TRUE,
         encoding = "UTF-8")
...
sjt.xtab(var.row = dados_cor$Doença_Card,
         var.col = dados_cor$Categ_BC_Desc,
         show.exp = TRUE,
         show.row.prc = TRUE,
         show.col.prc = TRUE,
         encoding = "UTF-8")
